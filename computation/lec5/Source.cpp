	...
	for (int i = splineNum; i < 2 * splineNum; i++)
	{
		for (int j = 0; j < 4 * splineNum; j++)
		{
			if (j % splineNum == i % splineNum)
				matrix[i][j] = pow((functionNodes[i % splineNum + 1].first - functionNodes[i % splineNum].first), (j / splineNum));
			else
				matrix[i][j] = 0;
		}
	...
	for (int i = 2 * splineNum; i < 3 * splineNum - 1; i++)
	{
		for (int j = 0; j < 4 * splineNum; j++)
		{
			if (j / splineNum == 1 && j % splineNum - 1 == i % splineNum)
				...
				matrix[i][j] = (j / splineNum) * pow((functionNodes[i % splineNum + 1].first - functionNodes[i % splineNum].first), (j / splineNum));
			else
				matrix[i][j] = 0;
		}
	}
